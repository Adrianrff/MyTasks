...
public class TasksDatabase extends SQLiteOpenHelper {
   ...
   private static final String COL_WHEN = "When";
         ...
         synchronized (MainActivity.sDataLock) {
            ...
            if (cursor.getCount() != 0 && cursor.moveToFirst()) {
               do {
                  ...
                  task.setWhen(cursor.getString(cursor.getColumnIndex(COL_WHEN)));
   ...
   public long addTaskToDataBase(LocalTask localTask) {
      ...
      cv.put(COL_WHEN, localTask.getWhen());
   ...
   public void addTasks(List<LocalTask> tasks) {
      ...
      for (int i = 0; i < tasks.size(); i++) {
         ...
         cv.put(COL_WHEN, tasks.get(i).getWhen());
   ...
   public int updateLocalTask(LocalTask modifiedTask, boolean updateReminders) {
      ...
      cv.put(COL_WHEN, modifiedTask.getWhen());
   ...
   public void setWhen(int id, String when) {
      db = getWritableDB();
      String selection = COL_INT_ID + " = ? ";
      String[] selectionArgs = {String.valueOf(id)};
      ContentValues cv = new ContentValues();
      cv.put(COL_WHEN, when);
      db.update(TABLE_NAME, cv, selection, selectionArgs);
      db.close();
   }

   public String getWhen(int id) {
      db = getWritableDB();
      String selection = COL_INT_ID + " = ? ";
      String when = null;
      String[] selectionArgs = {String.valueOf(id)};
      Cursor cursor = db.query(TABLE_NAME,new String[] {COL_WHEN},selection, selectionArgs,null, null, null);
      if (cursor != null && cursor.getCount() > 0) {
         cursor.moveToFirst();
         when = cursor.getString(cursor.getColumnIndex(COL_WHEN));
         cursor.close();
      }
      db.close();
      return when;
